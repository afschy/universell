...
async function getAllSellTransactions(user_id){
    const sql = `
    SELECT ORDER_ID, T.USER_ID AS BUYER_ID, U.NAME AS BUYER_NAME, T.POST_ID AS POST_ID, T.TIME AS TIME, QUANTITY, STATUS, AMOUNT, PHONE, ADDRESS, PR.NAME AS PRODUCTNAME, GET_FIRST_POST_IMAGE(T.POST_ID) AS IMAGE
    FROM TRANSACTION T
    JOIN POST P
...
async function getAllBuyTransactions(user_id){
    const sql = `
    SELECT ORDER_ID, T.TIME AS TIME, T.POST_ID AS POST_ID, QUANTITY, STATUS, AMOUNT, PHONE, ADDRESS, PR.NAME AS PRODUCT_NAME, GET_FIRST_POST_IMAGE(T.POST_ID)
    FROM TRANSACTION T
