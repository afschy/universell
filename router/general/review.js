...
router.post('/toggleupvote', async(req, res) =>{
    ...
    if(hasUpvotedReview == 1)
        review_api.addUpvote(req.session.user_id, review_id);
    else
        review_api.removeUpvote(req.session.user_id, review_id);
...
router.post('/newreview', async(req, res) =>{
    ...
    review_api.addReview(req.session.user_id, post_id, text, image, num_stars);
